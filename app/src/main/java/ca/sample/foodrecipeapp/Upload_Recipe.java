        ...
        FirebaseDatabase.getInstance().getReference("Recipe").child(myCurrentDateTime).setValue(foodData).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                ...
                if(task.isSuccessful()){
                    Log.d("FB", " Recipe added to Database");
                }
                else{
                    Log.d("FB", "onComplete: failed to store in DATABASE" + task.getException());
                }
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Log.d("FB", " Recipe not added to Database " + e.getMessage());
            }
        });
